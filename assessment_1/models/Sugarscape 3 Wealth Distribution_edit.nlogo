...
to turtle-setup ;; turtle procedure
  ...
  set max-age random-in-range ( max-max-age * 0.6 ) max-max-age
  set age 0
...
to go
  ...
  ask turtles [
    ...
    turtle-eat
    set age (age + 1)
    ;; (edit) check if cause of death is natural or of starvation
  ...
  ;; (edit) reset the starved ratio after the inital state has passed
  ...
  ;; (edit) calculate starved ratio, if none dead yet rate is 1
  carefully
...
@#$#@#$#@
<experiments>
  <experiment name="metab-vision-pop" repetitions="20" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
